...
def test_match_len_one(nornir):
    results = nornir.run(
        task=get_json_dict,
        tests=[test_jsonpath(path="$.type", assertion='is_equal_to', value='DeviceConfiguration')],
    )
...
def test_found_duplicate_host_data(nornir):
    results = nornir.run(
        task=get_json_dict,
        tests=[test_jsonpath(path="$.dns_search", assertion='contains', host_data='$..duplicate')],
    )
...
def test_path_not_found(nornir):
    results = nornir.run(
        task=get_json_dict,
        tests=[test_jsonpath(path="$.invalid", assertion='contains', host_data='$.domain')],
    )
...
def test_with_one_of_single_match(nornir):
    results = nornir.run(
        task=get_json_dict,
        tests=[test_jsonpath(path="$.interfaces..address", assertion='is_equal_to', value="192.168.1.1", one_of=True)],
    )
...
def test_without_one_of_single_match(nornir):
    results = nornir.run(
        task=get_json_dict,
        tests=[test_jsonpath(path="$.interfaces..address", assertion='is_equal_to', value="192.168.1.1")],
    )
...
def test_with_one_of_multi_match(nornir):
    results = nornir.run(
        task=get_json_dict,
        tests=[test_jsonpath(path="$.interfaces..mask", assertion='is_equal_to', value=24, one_of=True)],
    )
...
def test_without_one_of_multi_match_failed(nornir):
    results = nornir.run(
        task=get_json_dict,
        tests=[test_jsonpath(path="$.interfaces..mask", assertion='is_equal_to', value="24")],
    )
...
def test_without_one_of_multi_match_passed(nornir):
    results = nornir.run(
        task=get_json_dict,
        tests=[test_jsonpath(path="$.interfaces..mtu", assertion='is_equal_to', value="1500")],
    )
...
def test_dont_fail_task(nornir):
    results = nornir.run(
        task=get_json_dict,
        tests=[test_jsonpath(path="$.dns_servers", assertion='contains', value="8.8.8.8")],
    )
...
def test_fail_task(nornir):
    results = nornir.run(
        task=get_json_dict,
        tests=[test_jsonpath(path="$.dns_servers", assertion='contains', value="8.8.8.8", fail_task=True)],
    )
