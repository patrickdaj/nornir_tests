...
def test_single_match(single_host):
    results = single_host.run(
        ...
        tests=[
            t_regexp(
                regexp=r"Ethernet1 is up", assertion="is_not_none"
            )
        ],
    )
...
def test_multiple_match(single_host):
    results = single_host.run(
        ...
        tests=[
            t_regexp(
                regexp=r"Ethernet. is up", assertion="is_not_none"
            )
        ],
    )
    assert results["test"][0].tests[0].passed
...
def test_single_match_group(single_host):
    results = single_host.run(
        ...
        tests=[
            t_regexp(
                regexp=r"(.*?) is up", assertion="is_equal_to", one_of=True, value="Ethernet1"
            )
        ],
    )
    assert results["test"][0].tests[0].passed
...
def test_too_many_match_groups(single_host):
    results = single_host.run(
        ...
        tests=[
            t_regexp(
                regexp=r"(.*?) is (.*)", assertion="is_equal_to", one_of=True, value="Ethernet1"
            )
        ],
    )
